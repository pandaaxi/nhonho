...
Get-ChildItem -Recurse -File -Path "dist" -Filter "*windows-setup*.exe" | Copy-Item -Destination "out\Hiddify-Windows-Setup-x64.exe"
Get-ChildItem -Recurse -File -Path "dist" -Filter "*windows.msix" | Copy-Item -Destination "out\Hiddify-Windows-Setup-x64.msix"
